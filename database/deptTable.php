...
?>
        <tr class="text-center">
            <td><?= $index++ ?></td>
            <td><?= $row['unitName'] ?></td>
            <td><?= getFinancilaAll($row['unitID']) ?></td>
            <td><?=getArrears($row['unitID'])?></td>
            <td class="col-2">
                <a href="?page=financialhistory&unitid=<?= $row['unitID'] ?>&pid=<?= $_GET['pid'] ?>&search=<?= $searchParm ?>" class="btn btn-info btn-sm"><i class='bx bx-history'></i> ລາຍລະອຽດ</a>
            </td>
        </tr>
<?php
    }
...
function getFinancilaAll($unitID){
    require_once("./database/connectDB.php");
    $connnect = new connectDB();
    $db = $connnect->getConnection();
    $sql = "SELECT COALESCE(COUNT(FinancialID),0) AS Arrears FROM tb_financail WHERE UnitID=?";
    $stmt = $db->prepare($sql);
    $stmt->execute([$unitID]);
    $result = $stmt->fetchAll();
    return $result[0]['Arrears'];
